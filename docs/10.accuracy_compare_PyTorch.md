...
4. 运行命令：
   ...
   ```shell
   msprobe compare -i ./compare.json -o ./output -s -dm data_mapping.yaml
   ```
...
3. 运行命令：
   ...
   ```shell
   msprobe compare -i ./compare.json -o ./output
   ```
...
4. 运行命令:
   ...
   ```shell
   msprobe compare -i ./compare.json -o ./output -da
   ```
...
```Python
compare(input_param, output_path, stack_mode=False, auto_analyze=True, fuzzy_match=False, highlight=False)
```
...
| 参数名          | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 是否必选 |
|--------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| -------- |
| input_param  | 配置 dump 数据文件及目录，dict 类型。配置参数包括：<br>        "npu_path"：指定 NPU dump 目录下的 dump.json 文件。<br/>**配置示例**："npu_path": "./npu_dump/dump.json"。<br/>        "bench_path"：指定 CPU、GPU 或 NPU dump 目录下的 dump.json 文件。<br/>**配置示例**："bench_path": "./bench_dump/dump.json"。<br/>        "stack_path"：指定 NPU dump 目录下的 stack.json 文件。<br/>**配置示例**："stack_path": "./npu_dump/stack.json"。<br/>        "is_print_compare_log"：配置是否开启单个算子的日志打屏。<br/>**配置示例**：True 或 False。 | 是       |
...
| stack_mode   | 配置 stack_mode 的开关，bool 类型。仅当配置 stack_path 时需要，开启时比对结果呈现NPU_Stack_Info，关闭时不呈现。当不配置stack_path 时，自动识别是否存在stack.json，存在时呈现NPU_Stack_Info，否则不呈现。<br/>**配置示例**：stack_mode=True，默认为 False。                                                                                                                                                                                                                                                                                        | 否       |
