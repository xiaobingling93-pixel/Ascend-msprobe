...
模型分级可视化比对完成后，可通过tensorboard需安装[tb_graph_ascend插件](../plugins/tb_graph_ascend_instruct.md#1依赖安装)）启动端口，在浏览器页面查看模型结构和精度比对结果，请参考[分级可视化构图比对-启动tensorboard](../accuracy_compare/pytorch_visualization_instruct.md#4启动tensorboard)和[分级可视化构图比对-浏览器查看](../accuracy_compare/pytorch_visualization_instruct.md#5浏览器查看)。
