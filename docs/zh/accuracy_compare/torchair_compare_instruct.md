...
执行 `msprobe compare --target_path [GE dump data path] --golden_path [FX dump data path] --output_path [output path] --mode torchair`，在指定的 `output_path` 路径下输出比对结果csv文件，若不使用 `--output_path` 参数，则默认保存在当前目录下。
...
执行 `msprobe compare --target_path [GE dump data path] --golden_path [fusion off GE dump data path] --output_path [output path] --mode torchair`，在指定的 `output_path` 路径下输出比对结果csv文件，若不使用 `--output_path` 参数，则默认保存在当前目录下。
