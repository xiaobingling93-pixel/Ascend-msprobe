...
[tb_graph_ascend](../../../plugins/tensorboard-plugins/tb_graph_ascend#2-安装方式)
...
## 2.模型结构数据采集
请参见《[MindSpore场景精度数据采集](../dump/mindspore_data_dump_instruct.md)》。
...
| 参数名               | 说明                                                                         | 是否必选 |
|-------------------|----------------------------------------------------------------------------|------|
| npu_path   | 指定待调试侧比对路径，str类型。工具根据路径格式自动进行单rank比对、多rank批量比对或多step批量比对，具体格式参考[3.2 图构建和比对](#32-图构建和比对)。 | 是    |
