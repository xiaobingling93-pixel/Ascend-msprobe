...
当选择维度为Step时，如果是Megatron模型并行场景，可以使用[Megatron模型并行可视化功能](#megatron模型并行可视化)对照理解各个rank下采集的网络层数据与实际整网位置对应关系。
...
Megatron模型并行可视化是为[选择热力图展示](#选择热力图展示)功能提供Megatron模型并行场景各卡网络层与整网位置对应关系可视化能力。
