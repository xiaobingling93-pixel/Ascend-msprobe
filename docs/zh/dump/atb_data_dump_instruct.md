...
* 以真实数据模式采集精度数据时，工具将会直接保存op输入输出Tensor，输出结果件将会占用较大磁盘空间。
...
ATB dump功能用于ATB模型运行过程中的精度数据采集，包括模型结构信息、输入输出Tensor的真实数据或统计量数据。并且支持在模型运行过程中修改dump配置文件，实现动态dump。
...
| 参数 | 可选/必选 | 说明 |
| --- | --------- | --- |
| task         | 可选 | 指定dump任务，str类型，默认为"tensor"。可选值：<br/> "tensor"：采集op输入输出Tensor的真实数据；<br/> "statistics"：采集op输入输出Tensor的统计量数据；<br/> "all"：采集op输入输出Tensor的真实数据与统计量数据。 |
| dump_enable  | 可选 | 指定是否允许dump数据，bool类型，默认为false。可选值：<br/> true：允许采集op输入输出Tensor的真实数据或统计量数据；<br/> false：允许采集op输入输出Tensor的真实数据或统计量数据。 |
...
| filter_level | 可选 | 指定dump op输入输出Tensor的真实数据时的过滤等级，int类型，默认为1。该参数仅在指定layer级Operation，且"save_child"为true时生效。可选值：<br/> 0：采集op输入输出Tensor的真实数据时，不进行数据过滤；<br/> 1：采集op输入输出Tensor的真实数据时，相同Tensor仅保存一次；<br/> 2：在1基础上，过滤Kernel的输入输出Tensor。 |
...
|   |   |   |    |   |   └── ...
|   |   |   |    |   ...
|   |   |   |    |   ├── statistic.csv  # op输入输出Tensor的统计量数据
|   |   |   |    ├── 1  #  op执行轮次
|   |   |   |    |   └── ...
|   |   |   |    ...
