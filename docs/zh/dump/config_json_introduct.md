...
| risk_level        | 可选     | API风险级别过滤，str类型，仅PyTorch场景且level配置为L1以及mix时生效。可选参数：<br/>&#8226; "ALL"：dump所有API的数据，默认值。<br/>&#8226; "CORE"：仅dump核心（高风险，易出现精度问题）API的数据，包括融合计算、通信、精密计算等。<br/>&#8226; "FOCUS"：dump核心API和关注API的数据，排除低风险API（如reshape、transpose、permute、to、view等形状变换类API）。<br/>配置示例："risk_level": "CORE"。                                                                                                                                                                                                                                                                                                                                                                                             |
