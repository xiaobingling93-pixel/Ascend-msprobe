     ...
     class ModelRunner(ModelRunnerKVCacheMixin):
       ...
       def forward(
          ...
          if hasattr(self, 'debugger'):
             ...
             # 如果在SGLang框架中使用了dp（设置--dp-size大于1），则需要在start接口中配置rank_id参数。
...
3. 启动SGLang框架模型推理，开始采集数据
    ...
    - 离线模式
      ...
      ```
## 数据采集功能介绍
SGLang精度数据采集详细功能以及采集的dump数据结构与PyTorch场景一致，具体请参见[《PyTorch场景精度数据采集-数据采集功能介绍》](./pytorch_data_dump_instruct.md#数据采集功能介绍)
