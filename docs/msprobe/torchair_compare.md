...
dump 数据保存路径为 `{dump_path}/msit_ge_dump`。其中 `{dump_path}` 为用户通过 `set_ge_dump_config` 接口的'dump_path' 参数传入的路径，`msit_ge_dump`为 msprobe 工具自动创建的目录。
...
dump 数据保存路径为 `{dump_path}/msit_fx_dump`。其中 `{dump_path}` 为用户通过 `set_fx_dump_config` 接口的'dump_path' 参数传入的路径，`msit_fx_dump`为 msprobe 工具自动创建的目录。
...
```sh
# 使用 7.1.0 以上版本 PTA
msprobe compare --target_path ${dump_path}/msit_ge_dump --golden-path ${dump_path}/msit_fx_dump --mode torchair
```

```sh
# 使用 7.1.0 版本 PTA
msprobe compare --target_path ${dump_path}/msit_ge_dump --golden-path ${dump_path}/msit_fx_dump/data_dump --mode torchair
```

```sh
# 使用 7.1.0 以下版本 PTA
msprobe compare --target_path ${dump_path}/msit_ge_dump --golden-path data_dump --mode torchair
```
...
```sh
msprobe compare --target_path ${dump_path in GE dump}/msit_ge_dump --golden-path ${dump_path in fusion off GE dump}/msit_ge_dump --mode torchair
```
