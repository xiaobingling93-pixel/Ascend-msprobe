...
| -config 或 --config_path       | 指定离线预检操作过程中额外配置（包括黑名单、白名单等）的 [config.json](./02.config_introduction.md) 文件，默认未配置。config.json 文件的配置可参考[配置文件介绍](./02.config_introduction.md)。            | 否                                 |
...
数据默认会存盘到 './ut_error_data{timestamp}' 路径下，如有需要，用户可以通过 error_data_path 参数来配置保存路径，error_data_path 参数在 [config.json](../../../config.json) 文件或 [config.yaml](../../../pytorch/api_accuracy_checker/config.yaml) 文件配置，config.json 文件需要在 acc_check 操作时通过 -config 参数指定。
...
#### 3.1.2 API 预检黑名单和白名单
   ...
   - 配置 [config.json](../../../config.json) 文件，该文件需要在 acc_check 操作时通过 -config 参数指定。
