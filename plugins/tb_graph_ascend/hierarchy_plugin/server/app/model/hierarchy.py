...
class Hierarchy:
    def __init__(self, graph_type, repo, micro_step, rank, step):
        ...
        self.current_hierarchy = {root_node_name: self.get_basic_rende_info(root_node_name, node_info)}
        # 默认展开根节点
    ...
    def resize_hierarchy(self, current_name):
        ...
        for group_type, children in self.group_children(node.get("children")):
            if group_type == UNEXPAND_NODE:
                ...
                if len(rows) > 1:
                    max_child_width = max(
                        max_child_width, INNER_WIDTH * MAX_PER_ROW + (HORIZONTAL_SPACING * (MAX_PER_ROW - 1))
                    )
