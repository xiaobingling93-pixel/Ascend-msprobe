...
class GraphUtils:
    ...
    def find_config_files(run_name):
        ...
        try:
            if GraphUtils.safe_check_load_file_path(run, True):
                return [
                    file.name for file in dir_path.iterdir() if file.is_file() and file.name.endswith(".vis.config")
                ]
            else:
                return []
        except Exception as e:
    ...
    def validate_colors_param(colors_json: str):
        ...
        if len(colors_json) == 0:
            return False, GraphUtils.t("colorsEmpty"), {}
    ...
    @staticmethod
    def validate_and_process_parallel_merge(parallel_merge, data):
        """
        验证并处理parallel_merge参数

        参数:
            parallel_merge: 需要验证的并行合并参数
            data: 用于存储处理结果的字典

        返回:
            如果验证失败，返回错误结果字典；如果成功，返回None并更新data字典
        """

        def _validate_single_parallel_config(config, config_name):
            """
            验证单个并行配置(npu或bench)
            ...
            返回:
                如果验证失败，返回错误结果字典；否则返回None
            """
            if not isinstance(config, dict):
                return {"success": False, "error": f'parallel_{config_name} {GraphUtils.t("convertParamsError")}'}
            ...
            if config.get("order") and not GraphUtils.is_safe_string(config["order"]):
                return {"success": False, "error": f'parallel_{config_name}.order {GraphUtils.t("convertParamsError")}'}

            return {"success": True}

        if not isinstance(parallel_merge, dict):
            return {"success": False, "error": GraphUtils.t("convertParamsError")}

        npu_parallel_merge = parallel_merge.get("npu")
        bench_parallel_merge = parallel_merge.get("bench")

        # 验证npu并行参数
        npu_result = _validate_single_parallel_config(npu_parallel_merge, "npu")
        if not npu_result.get("success"):
            return npu_result
        data["parallel_param_n"] = npu_parallel_merge

        # 验证bench并行参数（如果存在）
        if bench_parallel_merge:
            bench_result = _validate_single_parallel_config(bench_parallel_merge, "bench")
