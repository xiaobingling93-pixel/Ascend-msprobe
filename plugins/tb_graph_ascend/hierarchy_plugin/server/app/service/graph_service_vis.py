...
class JsonGraphService(GraphServiceStrategy):
    ...
    def search_node_by_precision(self, meta_data, values):
        ...
        try:
            ...
            # 单图
            if not graph_data.get(NPU):
                node_list = GraphUtils.split_graph_data_by_microstep(
                    graph_data.get("node", {}), meta_data.get("microStep", -1)
                )
            # 多图
            else:
                node_list = GraphUtils.split_graph_data_by_microstep(
                    graph_data.get(NPU), meta_data.get("microStep", -1)
                )
        ...
        try:
            ...
            if task == "md5" or task == "summary":
                if is_unmatch_children:
                    match_result = MatchNodesController.process_task_delete_child_layer(
                        graph_data, npu_node_name, bench_node_name, task
                    )
                else:
