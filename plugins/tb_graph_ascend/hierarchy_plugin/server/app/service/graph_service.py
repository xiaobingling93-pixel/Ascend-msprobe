            ...
            for file in files:
                if file.endswith(".vis"):  # check for .vis extension
                    ...
                    if error and is_safe_check:
                        error_list.append({"run": run, "tag": tag, "info": f"Error: {error}"})
    ...
    def change_node_expand_state(node_info, meta_data):
        ...
        try:
            ...
            # NPU
                ...
                hierarchy = LayoutHierarchyController.change_expand_state(
                    node_name, graph_type, graph_data.get(NPU, {}), micro_step
                )
            # 标杆
            elif graph_type == BENCH:
                hierarchy = LayoutHierarchyController.change_expand_state(
                    node_name, graph_type, graph_data.get(BENCH, {}), micro_step
                )
            else:
    ...
    def get_node_info(node_info, meta_data):
        ...
        try:
            ...
            if not graph_data.get(NPU) or graph_type == SINGLE:
                ...
                matched_node_link = (
                    node.get("matched_node_link", []) if isinstance(node.get("matched_node_link", []), list) else []
                )
                matched_node_name = (
                    matched_node_link[-1].replace(matched_node_preifx, "", 1) if matched_node_link else None
                )
                matched_node = (
                    graph_data.get(matched_node_type, {}).get("node", {}).get(matched_node_name)
            ...
            if task == "md5" or task == "summary":
                if is_match_children:
                    result = MatchNodesController.process_task_add_child_layer(
                        graph_data, npu_node_name, bench_node_name, task
                    )
                    return result
                else:
        ...
        try:
            ...
            if task == "md5" or task == "summary":
                if is_unmatch_children:
                    result = MatchNodesController.process_task_delete_child_layer(
                        graph_data, npu_node_name, bench_node_name, task
                    )
                else:
